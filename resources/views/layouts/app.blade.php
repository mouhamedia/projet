...
<head>
    ...
    <style>
        ...
        /* Correction Navigation Glass */
        .glass {
            background: rgba(0, 77, 0, 0.98); /* Légèrement plus opaque pour le mobile */
...
<body class="bg-gray-50">
    ...
    <nav class="glass sticky top-0 z-[100] shadow-xl">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-20 md:h-24">
                
                <div class="flex flex-col">
                    <span class="text-gold text-xl md:text-3xl font-bold tracking-tight leading-none">
                        El Hadji Ahmad Dème
                    </span>
                ...
                <div class="md:hidden flex items-center">
                    <button id="menu-toggle" class="text-gold p-2 focus:outline-none" aria-label="Menu">
        ...
        <div id="mobile-menu" class="md:hidden bg-[#003300] px-6 border-t border-gold/20">
            <div class="flex flex-col space-y-2 mt-4">
                <a href="/" class="text-white py-4 border-b border-white/5 text-lg">Accueil</a>
                <a href="/presentation" class="text-white py-4 border-b border-white/5 text-lg">L'Œuvre</a>
                <a href="/actualites" class="text-white py-4 border-b border-white/5 text-lg">Actualités</a>
                <a href="/videos" class="text-white py-4 border-b border-white/5 text-lg">Médiathèque</a>
                
                @auth
                    @if(Auth::user()->is_admin)
                        <a href="/admin/dashboard" class="text-gold py-4 border-b border-white/5 font-bold italic">Tableau de Bord Admin</a>
                    @endif
                    <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form-mobile').submit();" class="text-red-400 py-4">Déconnexion</a>
                    <form id="logout-form-mobile" action="{{ route('logout') }}" method="POST" class="hidden">@csrf</form>
                @else
                    <a href="/contact" class="text-gold py-6 font-bold text-xl italic text-center border border-gold/30 rounded-xl mt-4">Nous contacter</a>
                @endauth
            </div>
        </div>
    </nav>
    ...
    <footer class="bg-[#002200] text-white border-t-4 border-gold mt-20 relative z-10">
